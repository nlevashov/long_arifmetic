...
int main ()
{
	...
	NUM a, b;
	int a_float_len = 0, b_float_len = 0;
	string s;

	getline(fin, s);
	int a_point_pos = s.find('.');
	if (a_point_pos != string::npos) {
		a_float_len = s.length() - a_point_pos - 1;
		s.erase(s.begin() + a_point_pos);
	}
	a = NUM(s);

	getline(fin, s);
	int b_point_pos = s.find('.');
	if (b_point_pos != string::npos) {
		b_float_len = s.length() - b_point_pos - 1;
		s.erase(s.begin() + b_point_pos);
	}
	b = NUM(s);

	for (int i = 0; i < b_float_len - a_float_len; i++) a *= 10;
	for (int i = 0; i < a_float_len - b_float_len; i++) b *= 10;
